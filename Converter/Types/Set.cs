	...
	public class Set : IMapStart
	{
		...
		public void Start(string prefix, MappedPropertyInfo item)
		{
			...
			if (subType == PropertyMappingType.ManyToMany)
			{
				...
				_builder.AddLine(string.Format(".{0}(\"{1}\")", KeyColumn.FluentNHibernateNames.ChildKeyColumn, childItem.ExplicitColumnName));
			}
			...
			_keyColumn.Add(set.inverse, item.ExplicitColumnName, subType);
